...
public:
    ...
    * \returns Size */
	...
	/** \brief Set the picture is playing or not
	* \param b The boolean to tell if it is playing */
	void SetPlay(bool b){ mPlay = b; }
    ...
    * \param size The new picture size */
	...
	void StoreSparty(std::shared_ptr<CActor> actor);
	void SotreHarold(std::shared_ptr<CActor> actor);
	/** \brief Get the actor Sparty
	* \returns actor sparty */
	std::shared_ptr<CActor> GetSparty(){ return mSparty; }
	/** \brief Get the actor Harold
	* \returns actor harold */
	std::shared_ptr<CActor> GetHarold(){ return mHarold; }
    ...
    */
	...
	/** \brief Get a pointer to the ParticleSystem
	* \returns Pointer to the ParticleSystem Object
	*/
	CParticleSystem *GetParticleSystem(){ return &mParticleSystem; }
	/** \brief Set the width of the snow
	*/
	void SetWidth(){ mParticleSystem.SetWidth(mSize.Width); }
	/** \brief Set the height of the snow
	*/
	void SetHeight(){ mParticleSystem.SetHeight(mSize.Height); }
private:
    /// The picture size
    Gdiplus::Size mSize = Gdiplus::Size(1100, 900);
	...
	std::shared_ptr<CActor> mSparty = nullptr;
	std::shared_ptr<CActor> mHarold = nullptr;
	
	/// The particle system to make the snow
	CParticleSystem mParticleSystem;

	/// Check if the window is playing or not
	bool mPlay = false;
};
